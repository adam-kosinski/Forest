  ...
  constructor(){
    ...
    this.coords = {
      x: 2+Math.floor(Math.random()*96),
		...
		this.search_target_size = 1 + Math.random()*1.5; //in gw units (1/100 of game div width) - width and height styling for this Thing's search target div, when hidden
  ...
  constructor(species){
    ...
    if(this.alive){
      if(species == "Pine"){
        ...
        if(Math.random() < 0.7){
          let n_pinecones = Math.floor(Math.random()**2 * 4); //exponent, more likely less visible
